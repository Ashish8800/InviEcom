...
const { ReceiveModel, InvoiceModel } = require("../../models");
...
async function create(req, res) {
  ...
  let purchaseReceiveObj = req.body;

  // generate a unique id for project
  const id = `PRE${generateId(9)}`;

  // add missing detail in the project object
  purchaseReceiveObj.id = id;
  purchaseReceiveObj.status = "active";
  purchaseReceiveObj.createdBy = ADMIN.id;
  purchaseReceiveObj.updatedBy = ADMIN.id;

  // Now try to create a new project
  try {
    await new ReceiveModel(purchaseReceiveObj).save();

    InvoiceModel.updateOne(
      {
        purchaseOrderId: purchaseReceiveObj.purchaseOrderId,
      },
      {
        $set: {
      ...
      (error, result) => {
        if (!error) {
          ...
          return requestFail(
            res,
            "Something went wrong, Can't create purchase receive now"
          );
        }
      }
    );
