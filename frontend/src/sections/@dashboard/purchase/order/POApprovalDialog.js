...
POApprovalDialog.propTypes = {
  open: PropTypes.bool,
  onClose: PropTypes.func,
};

export default function POApprovalDialog({ open, onClose, data }) {
  ...
  return (
    ...
    >
      <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
        <DialogTitle sx={{ paddingBottom: 0 }}>PO Approved</DialogTitle>

        <DialogContent dividers>
          <Stack spacing={2} sx={{mt:3}}>
            <Typography>
              You can download the Approved PO in PDF or send the same over the Email.
            </Typography>
            {data?.pdfUrl && <iframe title={data?.pdfUrl} height={400} src={data?.pdfUrl}></iframe>}
